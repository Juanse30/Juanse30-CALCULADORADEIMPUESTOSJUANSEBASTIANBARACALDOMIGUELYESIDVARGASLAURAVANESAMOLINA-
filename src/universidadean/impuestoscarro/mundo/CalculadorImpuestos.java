...
public class CalculadorImpuestos {
    ...
    private RangoImpuesto buscarRangoImpuesto(double valor) {
        ...
        for (int i = 0; i < rangosImpuesto.length; i++) { //se recorre todos los rangos de impuestos
            if(rangosImpuesto[i].contieneA(valor)){ //valido cual rango contiene el valor(precio)
    ...
    public double calcularPago(boolean descProntoPago, boolean descServicioPublico, boolean descTrasladoCuenta) {
        ...
        double pago = 0.0;
        double precio = darVehiculoActual().darPrecio(); //Obtenemos el precio del veiculo
        RangoImpuesto rango = buscarRangoImpuesto(precio); //Obtenemos el rango
        pago = precio * (rango.darPorcentaje()/100);  // multiplicamos el precio por el porcentaje que da el rango
